      ...
      def get_servers(role=nil, region=nil, cli_tags)
        ...
        ec2.servers.all.each do |instance|
          begin
            ...
            ######################################################################
             return server instance.public_ip_address, role || :web  if instance_tags.eql? cli_tags && instance.vpc_id.nil? || !instance.public_ip_address.nil?
             server instance.private_ip_address, role || :web if instance_tags.eql? cli_tags && instance.public_ip_address.nil?
          rescue => error
          end
        end
